...
object Function {
  ...
  def map3[T1, T2, T3, R](t1y: Try[T1], t2y: Try[T2], t3y: Try[T3])(f: (T1, T2, T3) => R): Try[R] = {
    t1y.flatMap(x => t2y.flatMap(y => t3y.map(z => f(x,y,z))))
  } // TO BE IMPLEMENTED
  ...
  def map7[T1, T2, T3, T4, T5, T6, T7, R](t1y: Try[T1], t2y: Try[T2], t3y: Try[T3], t4y: Try[T4], t5y: Try[T5], t6y: Try[T6], t7y: Try[T7])
    ...
    t1y.flatMap(x1 => t2y.flatMap(x2 => t3y.flatMap(x3 => t4y.flatMap(x4 => t5y.flatMap(x5 => t6y.flatMap(x6 => t7y.map(x7 => f(x1,x2,x3,x4,x5,x6,x7))))))))
  } // TO BE IMPLEMENTED
